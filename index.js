...
$(".foo").on("click", function () {
    let filterBrand = this.innerHTML;
    let filterData = data.filter((set) => (filterBrand === set.name) ? true : false);
...
$("#search").on("keyup", function(){
    ...
    let filtered = data.filter((obj) => {
        let newObj = JSON.stringify(obj).toLowerCase();
...
function buildProductView(arr) {
    ...
    arr.forEach((set) => {
        ...
        let perProduct =
            '<div class="card" style="width: 13rem; margin:30px; border:none;">' +
            ' <img src=" ' + set.image + ' ">' +
            '<div class="card-body">' +
